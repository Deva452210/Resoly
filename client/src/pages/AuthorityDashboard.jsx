...
const AuthorityDashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [complaintsRes, briefRes, intelligenceRes] = await Promise.all([
          api.get('/authority/complaints'),
          api.get('/ai/executive-summary'),
          api.get('/ai/civic-intelligence')
        ]);
        ...
        if (intelligenceRes.data) {
          setCivicIntelligence(intelligenceRes.data);
        }
      } catch (error) {
